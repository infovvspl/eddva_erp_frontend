import { ArrowRightLeft, BedDouble, Briefcase, Gift, GitBranch, Handshake, HeartHandshake, Mail, CalendarCheck, Gavel, DoorOpen, BadgeCheck, LayoutDashboard, GraduationCap, Building, ShoppingCart, Database, Shield, Key, Users, Utensils, Clock, UserPlus, Search, Monitor, PlayCircle, Receipt, CreditCard, Wallet, BarChart2, BookOpen, Folder, Settings, AlertTriangle, Trophy, Home, Swords, Award, Medal, Bell, Building2, UserCheck, LogIn, MessageSquare, Calendar, Boxes, Tag, MapPin, Truck, Package, ClipboardList, Tags, ClipboardCheck, Wrench, Contact, BellRing, Bus, Route } from 'lucide-react';
...
// Modules that already have their own landing page. Every other module gets the generic
// ModuleHomePage at its root path.
const MODULE_HOME: Record<string, string> = {
  '/sales-purchase': '/sales-purchase/dashboard',
  '/admission': '/admission/dashboard',
  '/hostel': '/hostel/dashboard',
  '/alumni': '/alumni/dashboard',
};

export function moduleHomePath(module: NavItem): string {
  return MODULE_HOME[module.path] ?? module.path;
}

/** The module (top-level nav entry with children) that owns this URL, if any. */
export function findActiveModule(pathname: string, items: NavItem[] = navItems): NavItem | undefined {
  return items.find(
    (item) => item.children && item.children.length > 0 && (pathname === item.path || pathname.startsWith(item.path + '/')),
  );
}
