...
import { getUserAssignments, revokeUserAssignment, resetUserAssignmentPassword } from '../../api/roles.api';
import { getApiErrorMessage } from '../../utils/errors';
import type { InventoryUserAssignment } from '../../types/role.types';

export default function UsersPage() {
  const [assignments, setAssignments] = useState<InventoryUserAssignment[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [search, setSearch] = useState('');
  const [resetTarget, setResetTarget] = useState<InventoryUserAssignment | null>(null);
  ...
  const handleRevoke = async (id: number, userName: string) => {
    if (!window.confirm(`Revoke Inventory access for "${userName}"?`)) {
      return;
    }
    try {
  ...
  const openResetModal = (assignment: InventoryUserAssignment) => {
  ...
  const handleResetPassword = async (e: React.FormEvent) => {
    ...
    try {
      setResetting(true);
      setResetError(null);
      const result = await resetUserAssignmentPassword(resetTarget.id, { new_password: newPassword });
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-600 mt-1">Manage inventory user role assignments</p>
        </div>
        <Link to="/inventory/users/new">
