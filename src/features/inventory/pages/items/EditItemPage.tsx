...
import { ITEM_STATUS_OPTIONS } from '../../constants/item.constants';
import type { InventoryItemUpdateData, InventoryItemStatus } from '../../types/item.types';
...
export default function EditItemPage() {
  ...
  const [formData, setFormData] = useState<InventoryItemUpdateData>({
    item_code: '',
    name: '',
    category_id: 0,
    unit_of_measure: '',
  ...
  async function loadData() {
    ...
    try {
      setLoading(true);
      const [item, allCategories] = await Promise.all([getItem(id), getCategories()]);
      setFormData({
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                />
                <p className="text-xs text-slate-500 mt-1">Item type cannot be changed after creation.</p>
              </div>
            </div>
            ...
            <div>
              ...
              <Select
                ...
                options={ITEM_STATUS_OPTIONS.map((opt) => ({ value: opt.value, label: opt.label }))}
              />
            </div>
