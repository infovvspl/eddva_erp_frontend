...
import type { TransportUserAssignment } from '../../types/role.types';

export default function UsersPage() {
  const [assignments, setAssignments] = useState<TransportUserAssignment[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [search, setSearch] = useState('');
  const [resetTarget, setResetTarget] = useState<TransportUserAssignment | null>(null);
  ...
  const handleRevoke = async (id: number, userName: string) => {
    if (!window.confirm(`Revoke Transport access for "${userName}"?`)) {
      return;
    }
    try {
  ...
  const openResetModal = (assignment: TransportUserAssignment) => {
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-600 mt-1">Manage transport user role assignments</p>
        </div>
        <Link to="/transport/users/new">
