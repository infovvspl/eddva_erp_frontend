...
import type { TransportRole, AssignTransportUserFormData } from '../../types/role.types';

export default function CreateUserPage() {
  const navigate = useNavigate();
  const [roles, setRoles] = useState<TransportRole[]>([]);
  const [formData, setFormData] = useState<AssignTransportUserFormData>({
    eddva_user_id: '',
    user_name: '',
    user_email: '',
    username: '',
    ...
    return (
      <div className="space-y-6">
        <div>
          ...
          <p className="text-slate-600 mt-1">Assign a user to a transport role</p>
        </div>
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-slate-900">Assign User</h1>
        <p className="text-slate-600 mt-1">Create a transport user assignment with login credentials</p>
      </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  placeholder="usr_dispatcher_001"
            ...
            <div className="flex gap-3 pt-4">
              <Button
                type="button"
                variant="ghost"
                onClick={() => navigate('/transport/users')}
                disabled={submitting}
              >
                Cancel
              </Button>
