...
import type { TransportPermissionFormData } from '../../types/role.types';

export default function CreatePermissionPage() {
  const navigate = useNavigate();
  const [formData, setFormData] = useState<TransportPermissionFormData>({
    resource: '',
    action: '',
    name: '',
    category: '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      navigate('/transport/permissions');
    } catch (err: any) {
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-slate-600 mt-1">Create a new transport permission</p>
      </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              ...
              <input
                ...
                placeholder="e.g., Export Fleet Report"
                className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#008BE9] focus:border-transparent"
                required
              />
            </div>
            ...
            <div>
              ...
              <textarea
                ...
                rows={3}
                placeholder="e.g., Allows exporting the fleet list to CSV"
                className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#008BE9] focus:border-transparent"
                required
              />
            </div>
            ...
            <div className="flex gap-3 pt-4">
              <Button
                type="button"
                variant="ghost"
                onClick={() => navigate('/transport/permissions')}
                disabled={submitting}
              >
                Cancel
              </Button>
