        ...
        <tbody>
          {salesReceipts.length === 0 ? (
            ...
            salesReceipts.map((salesReceipt) => (
              <tr key={salesReceipt.receipt_id} className="border-b border-slate-100 hover:bg-slate-50">
                <td className="py-3 px-4 font-medium text-slate-900">Receipt #{salesReceipt.receipt_id}</td>
                <td className="py-3 px-4">
                  <div className="flex items-center gap-2">
                    <Building2 className="h-4 w-4 text-slate-400" />
                    <div className="text-sm text-slate-900">{salesReceipt.invoice?.invoice_number || `Invoice #${salesReceipt.si_id}`}</div>
                  </div>
                </td>
                <td className="py-3 px-4 text-sm text-slate-600 hidden md:table-cell">
                  <div className="flex items-center gap-1">
                    <Calendar className="h-3 w-3 text-slate-400" />
                    {salesReceipt.receipt_date ? new Date(salesReceipt.receipt_date).toLocaleDateString() : '-'}
                  </div>
                </td>
                <td className="py-3 px-4 text-sm text-slate-600 hidden lg:table-cell">
                  <div className="flex items-center gap-1">
                    <IndianRupee className="h-3 w-3 text-slate-400" />
                    {Number(salesReceipt.amount).toFixed(2)}
                  </div>
                </td>
                <td className="py-3 px-4">
                ...
                <td className="py-3 px-4">
                  <div className="flex items-center gap-2">
                    <Link to={`/sales-purchase/sales-receipts/${salesReceipt.receipt_id}`}>
                      <button className="p-1.5 hover:bg-slate-100 rounded-lg text-slate-600" title="View">
                        <Eye className="h-4 w-4" />
                      </button>
                    </Link>
                    <Link to={`/sales-purchase/sales-receipts/${salesReceipt.receipt_id}/edit`}>
                    ...
                    <button
                      ...
                      title="Delete"
                      onClick={() => onDelete?.(salesReceipt.receipt_id)}
                    >
