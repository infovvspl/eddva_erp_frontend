...
export async function getPermissionsCatalog(): Promise<PermissionsCatalog> {
  const response = await axiosInstance.get('/sales-purchase/roles/permissions/catalog');
  return response.data.data;
}

export async function getMyPermissions(): Promise<RolePermission[]> {
  const response = await axiosInstance.get('/sales-purchase/roles/permissions/me');
  return response.data.data;
}
...
export async function createRole(data: RoleFormData): Promise<Role> {
  const response = await axiosInstance.post('/sales-purchase/roles', {
    ...data,
...
export async function updateRole(id: string | number, data: Partial<RoleFormData>): Promise<Role> {
  ...
  const response = await axiosInstance.patch(`/sales-purchase/roles/${id}`, payload);
  return response.data.data;
}
...
export async function getUserAssignments(): Promise<UserAssignment[]> {
  const response = await axiosInstance.get('/sales-purchase/roles/user-assignments');
  return response.data.data;
}

export async function createUserAssignment(data: UserAssignmentFormData): Promise<UserAssignment> {
  const response = await axiosInstance.post('/sales-purchase/roles/user-assignments', data);
  return response.data.data;
}

export async function revokeUserAssignment(id: string | number): Promise<void> {
  await axiosInstance.delete(`/sales-purchase/roles/user-assignments/${id}`);
}
...
export async function resetUserAssignmentPassword(
  ...
  const response = await axiosInstance.patch(`/sales-purchase/roles/user-assignments/${id}/password`, data);
  return response.data.data;
}
