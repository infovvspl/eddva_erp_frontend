...
import { createSport } from '../../api/sports.api';
...
export default function CreateSportPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(getApiErrorMessage(err, 'Failed to create sport'));
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-slate-900">Add Sport</h1>
        <p className="text-slate-600 mt-1">Add a new sport to the catalog</p>
      </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              ...
              <textarea
                ...
                rows={3}
                placeholder="e.g., Association football / soccer"
                className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#008BE9] focus:border-transparent"
              />
            </div>
