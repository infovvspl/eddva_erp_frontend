...
import { getPermissionsCatalog, getMyPermissions, createRole } from '../../api/sports.api';
import {
...
export default function CreateRolePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (grantablePermissions.length === 0) {
      setError('You can only assign permissions that your account already has in the sports module.');
      return;
    }
    ...
    return (
      <div className="space-y-6">
        <div>
          ...
          <p className="text-slate-600 mt-1">Create a new sports role with permissions</p>
        </div>
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="border-slate-200">
        <div className="p-6">
          {!config.apiToken?.trim() && !localStorage.getItem('accessToken') && !moduleSessions.sports.isAuthenticated() && (
            <div className="mb-4 bg-amber-50 border border-amber-200 text-amber-800 px-4 py-3 rounded-lg text-sm">
              ...
              The dummy login token cannot create sports roles.
            </div>
          )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#008BE9] focus:border-transparent"
                  placeholder="e.g. Manages teams, match fixtures, and results"
                  required
                />
              </div>
            </div>
