...
import type { UserAssignment } from '../../types/sports.types';
...
export default function UsersPage() {
  ...
  const handleRevoke = async (id: number, userName: string) => {
    if (!window.confirm(`Revoke sports access for "${userName}"?`)) {
      return;
    }
    try {
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-600 mt-1">Manage sports user role assignments</p>
        </div>
        <Link to="/sports/users/new">
