...
import { getTournament, updateTournament, getSports, getVenues } from '../../api/sports.api';
...
export default function EditTournamentPage() {
  ...
  async function loadData() {
    ...
    try {
      ...
      setError(getApiErrorMessage(err, 'Failed to load tournament'));
    } finally {
      setLoading(false);
    }
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      navigate(`/sports/tournaments/${id}`);
    } catch (err: any) {
      if (err.response?.status === 401) {
        return;
      }
      setError(getApiErrorMessage(err, 'Failed to update tournament'));
    } finally {
      setSubmitting(false);
    }
  };
  ...
  if (loading) {
    return (
      <div className="space-y-6">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">Edit Tournament</h1>
          <p className="text-slate-600 mt-1">Update tournament details</p>
        </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="flex gap-3 pt-4">
              <Button
                type="button"
                variant="ghost"
                onClick={() => navigate(`/sports/tournaments/${id}`)}
                disabled={submitting}
              >
                Cancel
              </Button>
