...
import { admissionLoginSchema, type AdmissionLoginFormData } from '../../schemas/admission-login.schema';
import { useAdmissionLogin } from '../../hooks/useAdmissionLogin';
...
export default function AdmissionLoginForm() {
  ...
  const {
    ...
    handleSubmit,
    formState: { errors },
  } = useForm<AdmissionLoginFormData>({
    resolver: zodResolver(admissionLoginSchema),
  });

  const { login, isLoading, error } = useAdmissionLogin();

  const onSubmit = (data: AdmissionLoginFormData) => {
    login(data);
  };
    ...
    <div className="w-full">
      <div className="text-center mb-8">
        <h2 className="text-2xl font-bold tracking-tight text-slate-800">
          Admission Staff Sign In
        </h2>
        <p className="text-sm text-slate-500 mt-1.5">
          Please enter your admission credentials to continue
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-5" noValidate>
        ...
        <FormField label="Institute ID (only if asked)" error={errors.institute_id?.message}>
          <Input
            type="text"
            placeholder="Leave empty unless your username exists in more than one institute"
            disabled={isLoading}
            className="px-4 py-2.5 w-full rounded-lg bg-slate-50/50 border border-slate-200 text-slate-800 placeholder:text-slate-400 focus:bg-white focus:outline-none focus:ring-2 focus:ring-[#008BE9]/20 focus:border-[#008BE9] transition-all"
