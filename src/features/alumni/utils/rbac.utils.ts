...
import { getAlumniToken, getCachedIsAlumniInstituteAdmin } from './ssoSession';
import type { RolePermission } from '../types/alumni.types';

export function isCurrentUserInstituteAdmin(): boolean {
  return getCachedIsAlumniInstituteAdmin();
}

// Reactive version: the Alumni session token is exchanged
...
export function useIsInstituteAdmin(): boolean {
  const [isAdmin, setIsAdmin] = useState(getCachedIsAlumniInstituteAdmin());

  useEffect(() => {
    let cancelled = false;
    getAlumniToken()
      .then(() => {
        if (!cancelled) setIsAdmin(getCachedIsAlumniInstituteAdmin());
      })
      .catch(() => {
