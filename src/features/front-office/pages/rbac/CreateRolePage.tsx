...
export default function CreateRolePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (grantablePermissions.length === 0) {
      setError('You can only assign permissions that your account already has in the front office module.');
      return;
    }
    ...
    return (
      <div className="space-y-6">
        <div>
          ...
          <p className="text-slate-600 mt-1">Create a new front office role with permissions</p>
        </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#008BE9] focus:border-transparent"
                  placeholder="e.g. Registers visitors, manages appointments, logs enquiries and complaints"
                  required
                />
              </div>
            </div>
            ...
            <div className="flex gap-3 pt-4">
              <Button
                type="button"
                variant="ghost"
                onClick={() => navigate('/front-office/roles')}
                disabled={submitting}
              >
                Cancel
              </Button>
