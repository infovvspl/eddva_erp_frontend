...
import { getCanteenToken, getCachedIsCanteenInstituteAdmin } from './ssoSession';
import type { RolePermission } from '../types/canteen.types';

export function isCurrentUserInstituteAdmin(): boolean {
  return getCachedIsCanteenInstituteAdmin();
}

// Reactive version: the Canteen session token is exchanged
...
export function useIsInstituteAdmin(): boolean {
  const [isAdmin, setIsAdmin] = useState(getCachedIsCanteenInstituteAdmin());

  useEffect(() => {
    let cancelled = false;
    getCanteenToken()
      .then(() => {
        if (!cancelled) setIsAdmin(getCachedIsCanteenInstituteAdmin());
      })
      .catch(() => {
