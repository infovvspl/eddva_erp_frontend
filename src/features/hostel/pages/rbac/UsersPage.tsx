...
import { getHostelUserAssignments, deleteHostelUserAssignment, resetHostelUserAssignmentPassword } from '../../api/roles.api';
import { getApiErrorMessage } from '../../utils/errors';
import type { HostelUserAssignment } from '../../types/role.types';

export default function UsersPage() {
  const [assignments, setAssignments] = useState<HostelUserAssignment[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [search, setSearch] = useState('');
  const [resetTarget, setResetTarget] = useState<HostelUserAssignment | null>(null);
  ...
  async function loadAssignments() {
    try {
      setLoading(true);
      const data = await getHostelUserAssignments();
      setAssignments(data);
    } catch (err: any) {
  ...
  const handleDelete = async (id: number, userName: string) => {
    if (!window.confirm(`Revoke Hostel access for "${userName}"?`)) {
      return;
    }
    try {
      await deleteHostelUserAssignment(id);
  ...
  const openResetModal = (assignment: HostelUserAssignment) => {
  ...
  const handleResetPassword = async (e: React.FormEvent) => {
    ...
    try {
      setResetting(true);
      setResetError(null);
      const result = await resetHostelUserAssignmentPassword(resetTarget.id, { new_password: newPassword });
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">Hostel Users</h1>
          <p className="text-slate-600 mt-1">Manage hostel user role assignments</p>
        </div>
        <Link to="/hostel/users/new">
                      ...
                      <td className="py-3 px-4 text-right">
                        <div className="flex items-center justify-end gap-2">
                          ...
                          <Button
                            variant="ghost"
                            size="sm"
                            onClick={() => handleDelete(assignment.id, assignment.user_name)}
